"""
================================================================================
APPLEGUARD AI - APPLE INSPECTION SYSTEM
================================================================================
...
IS_CLOUD = os.getenv("STREAMLIT_SHARING_MODE") is not None or os.getenv("STREAMLIT_RUNTIME_ENV") == "cloud"
...
CLASS_INFO = {
    ...
    3: {'name': 'Black Pox', 'color': '#FF9800', 'emoji': '⚠️'},
...
if st.session_state.model is None and DEFAULT_MODEL_PATH.exists():
    try:
        st.session_state.model = YOLO(str(DEFAULT_MODEL_PATH))
    except Exception as e:
        pass  # Will show warning in sidebar if model fails to load
...
with col1:
    ...
    if file_type == "Image":
        ...
        # Video upload disabled on Streamlit Cloud
        if IS_CLOUD:
            st.warning("⚠️ Video analysis is only available for local deployment. Please use image upload on Streamlit Cloud.")
            uploaded_file = None
        else:
    ...
    if uploaded_file:
        ...
        if not st.session_state.model:
            st.warning("⚠️ Load model from sidebar first")
...
with col2:
    ...
    if st.session_state.results:
        ...
        st.markdown(f'''
        <div class="info-box">
            <strong>⚙️ Config:</strong> Confidence: {confidence} | IOU: {iou} | Avg: {results['avg_confidence']:.3f}
        </div>
        ''', unsafe_allow_html=True)
    else:
...
<div style='text-align: center; color: #666; padding: 1rem 0;'>
    <h4>AppleGuard AI</h4>
    <p>An apple inspection tool for identifying common apple infections</p>
